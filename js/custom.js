...
//Assign Branch
$.puertosend("#assignBranchform", "assignBranch");
...
// assign branch too user

$('.assign_branch').on('click',function(){
	var id = $(this).data("id");
...
$.puertosend("#addrider", "addrider", "alert");
...
$('#payment-form').on('submit', function(e){
	...
	const name = ths.find('input[name="name"]').val();
	const email = ths.find('input[name="email"]').val();
	const amount = ths.find('input[name="amount"]').val();
	const phone = ths.find('input[name="phone"]').val();
	const address = ths.find('input[name="address"]').val();
	const city = ths.find('input[name="city"]').val();
	const state = ths.find('input[name="state"]').val();
	const country = ths.find('input[name="country"]').val();
	const zip = ths.find('input[name="zip"]').val();
...
// 		tx_ref: "12Bas-"+generateTransactionId() + "-payment",
// 		amount: data.amount,
// 		currency: "NGN",
// 		payment_options: "card, banktransfer, ussd",
// 		redirect_url: "https://unsinkableship.com/redirect",
// 		meta: {
// 			consumer_id: 23,
...
function payment(data){
	FlutterwaveCheckout({
		public_key: "FLWPUBK_TEST-e6a2fdc7c99903def336ab7d047ad6f7-X",
		tx_ref: "12Bas-"+generateTransactionId(),
		amount: data ? data.amount : '4000',
		currency: "NGN",
		...
		customer: {
		  email: data ? data.email : 'babajide234@gmail.com	',
		  phone_number: data ? data.phone : '09055548937',
		  name: data ? data.name : 'John doe',
		},
		customizations: {
		  title: "12Baskets",
...
console.log("init: ",country_code);
console.log("init: ",country_name);
...
function verifyTransactionOnBackend(transactionId) {
	...
	// Payment was successful
		...
		var tx_ref = transactionId.tx_ref;
		var amount = transactionId.amount;
		var currency = transactionId.currency;
		var name = transactionId.name;
		var email = transactionId.email;
		var phone = transactionId.phone;
		var tx_id = transactionId.transaction_id;
		var address = transactionId.address;
		var city = transactionId.city;
		var state = transactionId.state;
		var postal_code = transactionId.postal_code;
		var country = transactionId.country;
		var gender = transactionId.gender;
		...
		// Send AJAX verification request to backend
		// $.post(path+"/ajax.php?pg=stripe", form.serialize(), function(response){
		$.post(path+"/flutterwave.php", transactionId, function(response){
			if(response){
				// $.puerto_confirm("Success!", transactionId.status , "green");
	...
	// Payment was not successful
		$.puerto_confirm("Error!", transactionId.status, "red");
		btn.html(btxt).prop('disabled', false);
	}
...
$('.location').on('click', function(){
	...
	var url = path+"/ajax.php?pg=getgps"
	// var url = path+"/getLocation.php";

	$.post(url,{id:id},function(data){
		data = data.data[0];
		var lat = parseFloat(data.tck_lat);
		var lng = parseFloat(data.tck_lng);
		$('.tracktime').append(data.tck_time);
		console.log('From db',data);
		const initialPosition = { lat: data ? lat : 59.32, lng: data ? lng : 17.84 };
