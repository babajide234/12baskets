...
switch ($pg) {
	case 'addtocart':
		include __DIR__ . "/ajax/addtocart.php";
		break;
	case 'getmodalitem':
		include __DIR__ . "/ajax/getmodalitem.php";
		break;
	case 'sendpaypalitem':
		include __DIR__ . "/ajax/sendpaypalitem.php";
		break;
	case 'sendpaypalplan':
		include __DIR__ . "/ajax/sendpaypalplan.php";
		break;
	case 'sendplans':
		include __DIR__ . "/ajax/sendplans.php";
		break;
	case 'sendrestaurant':
		include __DIR__ . "/ajax/sendrestaurant.php";
		break;
	case 'sendcuisine':
		include __DIR__ . "/ajax/sendcuisine.php";
		break;
	case 'sendmenu':
		include __DIR__ . "/ajax/sendmenu.php";
		break;
	case 'senditem':
		include __DIR__ . "/ajax/senditem.php";
		break;
	case 'imageupload':
		include __DIR__ . "/ajax/imageupload.php";
		break;
	case 'multiupload':
		include __DIR__ . "/ajax/multiupload.php";
		break;
	...
	case 'sendsignup':
		...
		break;
	case 'addrider':
		include __DIR__ . "/ajax/addrider.php";
		break;
	case 'senduserdetails':
		include __DIR__ . "/ajax/senduserdetails.php";
		break;
	case 'sendpassword':
		include __DIR__ . "/ajax/sendpassword.php";
		break;
	case 'sendreview':
		include __DIR__ . "/ajax/sendreview.php";
		break;


	case 'sendsettings':
		include __DIR__ . "/ajax/sendsettings.php";
		break;
	case 'sendpage':
		include __DIR__ . "/ajax/sendpage.php";
		break;
	...
	case 'updategps':
		if ($_SERVER['REQUEST_METHOD'] === 'POST' && us_level) {
			...
			$id = sc_sec($_SESSION['login']);
			$lng = sc_sec($_POST["lng"]);
			$lat = sc_sec($_POST["lat"]);
			...
			if(empty($id) || empty($lng) || empty($lat)) {
				...
				$db->query("REPLACE INTO `pl_gps_track` (`rider_id`,  `tck_time`, `tck_lng`, `tck_lat`) VALUES ('{$id}','" . date("Y-m-d H:i:s") . "', '{$lng}', '{$lat}')");
	...
	case 'getgps':
		if ($_SERVER['REQUEST_METHOD'] === 'POST' && us_level) {
			...
			if(empty($id)) {
				...
				$sql = $db->query("SELECT * FROM `pl_gps_track` WHERE `rider_id`= {$id}") or ($db->error);
				if ($sql->num_rows) {
	...
	case 'assignBranch':
		if ($_SERVER['REQUEST_METHOD'] === 'POST' && us_level) {
			...
			$branchID = sc_sec($_POST["asn_branch"]) ?? null;
			$userID = sc_sec($_POST["asn_manager"]) ?? null;

			if(empty($branchID) || empty($userID) ) {
				$alert = [
					'type'  =>'danger',
					'alert' => false
				];
			}else{
				$db->query("UPDATE `pl_users` SET `branch`= {$branchID} WHERE id = {$userID}") or ($db->error);
				if ($db->affected_rows > 0) {
					$alert = [
						'type'  =>'success',
						'alert' => "Successfully Asigned Branch Manager",
					];
				} else {
					$alert = [
						'type'  =>'danger',
						'alert' => fh_alerts("No user selected")
					];
				}
			}
			echo json_encode($alert);
