...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
	if(!us_level || us_level == 6){
		...
		$reg_role   = isset($_POST['reg_role']) ? sc_sec($_POST['reg_role'])  : '';
		
	
		if(empty($reg_name) || empty($reg_pass) || empty($reg_email) || empty($reg_role)){
			$alert = [
				'type'  =>'danger',
				'alert' => fh_alerts($lang['signup']['alert']['required'])
			];
		} elseif(!preg_match('/^[\p{L}\p{N}]+$/u',$reg_name)){
			$alert = [
				'type'  =>'danger',
			...
			$data = [
				...
				'date'       => time(),
				'level'      => !empty($reg_role) ? $reg_role  : "1",
			];
